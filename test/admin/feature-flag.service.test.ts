...
describe("FeatureFlagService - default payloads + getPayload", () => {
   ...
   it("getPayload merges the stored JSON over the provided defaults", async () => {
      const { service, repo } = build();
      repo.findOne.mockResolvedValue({ namespace: NAMESPACE, key: "some_flag", enabled: true, payload: JSON.stringify({ accessThreshold: 99 }) });
      const cfg = await service.getPayload(NAMESPACE, "some_flag", { accessThreshold: 5, ttlSeconds: 3600 });
      expect(cfg).toEqual({ accessThreshold: 99, ttlSeconds: 3600 });
   });

   it("getPayload falls back to defaults on invalid JSON", async () => {
      const { service, repo } = build();
      repo.findOne.mockResolvedValue({ namespace: NAMESPACE, key: "some_flag", enabled: true, payload: "{not json" });
      const defaults = { accessThreshold: 5 };
      expect(await service.getPayload(NAMESPACE, "some_flag", defaults)).toEqual(defaults);
   });
});
