...
export class FilesService {
  ...
  constructor (
    ...
    this.dirService = new DirectoriesService(userService, this, uploadedFileRepo, searchStateRepo, logger, fs)
    ...
    arrayOfFiles.forEach((relativePath) => {
      ...
      if (ext == '.jpg' || ext == '.jpeg' || ext == '.png' || ext == '.gif') used_data.images += size
        ...
        ext == '.pdf' ||
                ...
                ext == '.txt' ||
                ext == '.rtf'
      ) { used_data.documents += size } else used_data.other += size
    })
  ...
  public async createMetaFolderIfNotExists (userId: number): Promise<string> {
    ...
    if (!this.fs.existsSync(path.join(dir, FilesService.THUMBNAILS_FOLDER_NAME))) { this.fs.mkdirSync(path.join(dir, FilesService.THUMBNAILS_FOLDER_NAME), { recursive: true }) }
