...
class HomeController extends Controller
{
     ...
     * @throws \Exception
     */
    public function index(Request $request, APIController $controller, FileServiceProvider $provider)
    {
        // See if it is a file or not (to preview)
        $path = $request->get("path") ?? $provider->getCloudPath();

        $provider->verifyIfPermissionToModify($path);
