...
describe('FilesService', () => {
  ...
  describe('upload', () => {
    ...
    it('should handle file replacement and invalidate thumbnails if file exists', async () => {
      ...
      // Mock redis cache
      cache.store.getClient().scan = jest
        .fn()
        .mockReturnValue([
          '0',
          [
      ...
      const thumbailAbsolute = (path: string) =>
                `${process.env.CLOUD_DIR}/${user.email}/${FilesService.METADATA_FOLDER_NAME}/${FilesService.THUMBNAILS_FOLDER_NAME}/${path}`
      ...
      // Redis clear
      expect(cache.store.getClient().del).toHaveBeenCalledWith(
        ThumbnailCacheInterceptor.getCacheKey(user.id, existingFile.absolute_path),
        ThumbnailCacheInterceptor.getCacheKey(user.id, existingFile.absolute_path) + '_MIME'
      )
    })
  ...
  describe('toThumbnail', () => {
    ...
    it('should throw an error if the file does not exist', async () => {
      ...
      await expect(service.toThumbnail('path/to/file.png', 1)).rejects.toThrowError(
                `${process.env.CLOUD_DIR}/${user.email}/path/to/file.png does not exist`
      )
    })
    ...
    it('should return the existing thumbnail if already generated', async () => {
      ...
      service.createMetaFolderIfNotExists = jest
        .fn()
        .mockResolvedValue(`${process.env.CLOUD_DIR}/${user.email}/${FilesService.METADATA_FOLDER_NAME}`)
      ...
      expect(fs.createReadStream).toHaveBeenCalledWith(
                `${process.env.CLOUD_DIR}/${user.email}/${FilesService.METADATA_FOLDER_NAME}/${FilesService.THUMBNAILS_FOLDER_NAME}/1_100x100.jpg`
      )
    })
    ...
    it('should create a new thumbnail if none exists for image file', async () => {
      ...
      jest.spyOn(fs, 'existsSync').mockImplementation(
        (path: string) => !path.includes(FilesService.THUMBNAILS_FOLDER_NAME)
      )
