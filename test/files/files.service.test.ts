...
describe('FilesService', () => {
    ...
    describe('upload', () => {
        ...
        it('should handle file replacement and invalidate thumbnails if file exists', async () => {
            ...
            userService.getById = jest.fn().mockResolvedValue(user);
            service.getUsedData = jest.fn().mockResolvedValue(usedData);
            service.getUserRootPath = jest.fn().mockResolvedValue(`${process.env.CLOUD_DIR}/${user.email}`);
            service.isOwner = jest.fn().mockResolvedValue(true);
            ...
            fs.readdirSync = jest.fn().mockReturnValue([
                `${existingFile.id}_100x120.jpg`,
                `${existingFile.id}_100xundefined.jpg`,
            ...
            // thumbnail delation 
            const thumbailAbsolute = (path: string) => `${process.env.CLOUD_DIR}/${user.email}/${FilesService.METADATA_FOLDER_NAME}/${FilesService.THUMBNAILS_FOLDER_NAME}/${path}`;
    ...
    describe('delete', () => {
        ...
        it('should delete the file if the user is the owner', async () => {
            ...
            const absolutePath = `${process.env.CLOUD_DIR}/${user.email}/path/to/file`;
        ...
        it('should delete all thumbnails associated with the file', async () => {
            const user = { id: 1, email: "cait@queen.com" };
            const absolutePath = `${process.env.CLOUD_DIR}/${user.email}/cait.jpg`;
            ...
            fs.unlinkSync = jest.fn();
            fs.readdirSync = jest.fn().mockReturnValue([
                `${uploadedFile.id}_100x100.jpg`,
                `${uploadedFile.id}_100xundefined.jpg`,
                `24_100x100.jpg`, // <--- Shouldn't be deleted!!
            ]); // <--- this function is called by invalidateThumbnailsFor
    ...
    describe('toThumbnail', () => {
        ...
        it('should throw an error if the user does not have permission', async () => {
            ...
            await expect(service.toThumbnail('test/path', 1))
                .rejects
                .toThrowError("You don't have permission to access this file");
        });
        ...
        it('should throw an error if the file does not exist', async () => {
            ...
            await expect(service.toThumbnail('path/to/file.png', 1)).rejects.toThrowError(`${process.env.CLOUD_DIR}/${user.email}/path/to/file.png does not exist`);
        });
        ...
        it('should return the existing thumbnail if already generated', async () => {
            ...
            service.createMetaFolderIfNotExists = jest.fn().mockResolvedValue(`${process.env.CLOUD_DIR}/${user.email}/${FilesService.METADATA_FOLDER_NAME}`);
            ...
            expect(fs.createReadStream)
                .toHaveBeenCalledWith(`${process.env.CLOUD_DIR}/${user.email}/${FilesService.METADATA_FOLDER_NAME}/${FilesService.THUMBNAILS_FOLDER_NAME}/1_100x100.jpg`);
        });
        ...
        it('should create a new thumbnail if none exists for image file', async () => {
            ...
            jest.spyOn(fs, "existsSync").mockImplementation((path: string) => !path.includes(FilesService.THUMBNAILS_FOLDER_NAME))

            uploadedFileRepo.findOne = jest.fn().mockResolvedValue(mockFile);
            const metaDir = `${process.env.CLOUD_DIR}/${user.email}/${FilesService.METADATA_FOLDER_NAME}`;
        ...
        it('should handle unsupported mime types gracefully', async () => {
            ...
            await expect(service.toThumbnail('test/path', 1, 100, 100))
                .resolves.toBe("data");
        });
    ...
    describe('isOwner', () => {
        ...
        it('should return true if user is the owner (sanitized relative path is valid)', async () => {
            const user = { id: 1, email: "cait@queen.com" };
            const absolutePath = `${process.env.CLOUD_DIR}/${user.email}/files`;
        ...
        it('should return false if user is trying to access a file outside their root directory', async () => {
            const user = { id: 1, email: "cait@queen.com" };
            const absolutePath = `${process.env.CLOUD_DIR}/other@shado.com/files`;
            ...
            expect(logger.log).toHaveBeenCalledWith(
                expect.stringContaining(`Not owner of ${process.env.CLOUD_DIR}/other@shado.com/files`)
            );
        });
        ...
        it('should return false if the relative path includes ".." and tries to go outside the root path', async () => {
            ...
            const absolutePath = `${process.env.CLOUD_DIR}/${user.email}/../other@shado.com/file.txt`;
            ...
            expect(result).toBe(false);
            expect(logger.log).toHaveBeenCalledWith(
                expect.stringContaining(`Not owner of ${absolutePath}`)
            );
        });

        it('should return true if the sanitized relative path is empty (path points directly to the root)', async () => {
            const user = { id: 1, email: "cait@queen.com" };
            const absolutePath = `${process.env.CLOUD_DIR}/${user.email}`;
        ...
        it('should return false if path traversal ".." is used multiple times to go out of the root path', async () => {
            ...
            const absolutePath = `${process.env.CLOUD_DIR}/${user.email}/../../etc/passwd`;
